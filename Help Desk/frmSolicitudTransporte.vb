Public Class frmSolicitudTransporte
    Sub LlenaServicios()
        Try
            ...
            StrSql = "SELECT * FROM TRANSP_SERVICIOS ORDER BY ID ASC"
    ...
    Private Sub btnOk_Click(sender As Object, e As EventArgs) Handles btnOk.Click
        If cmbServicios.Text = "" Then
            MsgBox("Debe seleccionar el servicio que desea", vbInformation)
            cmbServicios.Focus()
            Return
        End If
        If Len(Trim(txtDetalle.Text)) = 0 Then
            MsgBox("Debe dar mas detalles sobre el servicio que solicita", vbInformation)
            txtDetalle.Focus()
            Return
        End If
        ...
        Try
            ...
            StrSql = "INSERT INTO TRANSP_SOLICITUDES(NO_TICKET, ID_USUARIO, ID_DEPARTAMENTO," _
            ...
            & Format(Now, "HH:mm") & "'," & cmbIdServicio.Text & ",'" & Trim(txtDetalle.Text) & "'," & 1 & ")"
            objCmd = New SqlClient.SqlCommand(StrSql, Cnn)
            ...
            If RA > 0 Then
                ...
                StrSql = "UPDATE TRANSP_ALERTAS SET ESTADO = 1"
                objCmd = New SqlClient.SqlCommand(StrSql, Cnn)
                RA = objCmd.ExecuteNonQuery()
            ...
            Else
                MsgBox("No fue posible enviar la solicitud a transportación", vbCritical)
                Return
            End If
